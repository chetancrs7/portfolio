...
/**
 * URL-driven article search. The uncontrolled input debounces into the `q`
 * param so results stay server-rendered and shareable; the category filter is
 * preserved. No search backend — the collection is small enough to filter on
 * the server per request.
 */
export function WritingSearch({ className }: WritingSearchProps) {
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      />
      <input
        aria-label="Search writing by title, tag, or category"
        autoComplete="off"
